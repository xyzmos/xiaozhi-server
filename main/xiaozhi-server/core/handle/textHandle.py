...
class TextMessageHandler:
    ...
    async def _process_user_text(self, session_id: str, text: str):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).debug(
                f"开始处理用户文本: session_id={session_id}, text={text}"
            )
            ...
            task = asyncio.create_task(
                dialogue_service.process_user_input(session_id, text)
            )
        ...
        if state == "start":
            ...
            try:
                asr_service = self.container.resolve(
                    "asr_service", session_id=session_id
                )
            ...
            if "text" in msg_json:
                ...
                # 检查是否是唤醒词
                is_wakeup_words = filtered_text in context.get_config(
                    "wakeup_words", []
                )
                ...
                if is_wakeup_words and not enable_greeting:
                    ...
                    # 触发对话处理 - 通过意图处理器
                    self.logger.bind(tag=TAG).debug(
                        f"准备触发对话处理: text={original_text}, is_wakeup={is_wakeup_words}"
                    )
                    import asyncio

                    asyncio.create_task(
                        self._process_user_text(session_id, original_text)
                    )

    async def _handle_iot_message(self, session_id: str, msg_json: Dict[str, Any]):
        """处理IOT消息"""
        import asyncio
        from core.providers.tools.device_iot import (
            handleIotStatus,
    ...
    async def _handle_mcp_message(self, session_id: str, msg_json: Dict[str, Any]):
        ...
        try:
            ...
            if "payload" in msg_json and context.mcp_client:
                asyncio.create_task(
                    handle_mcp_message(context, context.mcp_client, msg_json["payload"])
                )
        except Exception as e:
    ...
    async def _handle_server_message(self, session_id: str, msg_json: Dict[str, Any]):
        ...
        if post_secret != secret:
            await ws_transport.send_json(
                session_id,
                {"type": "server", "status": "error", "message": "服务器密钥验证失败"},
            )
            return
