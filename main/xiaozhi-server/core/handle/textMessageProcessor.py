    ...
    async def process_message(self, conn, message: str, session_context: SessionContext) -> None:
        ...
        try:
            ...
            if isinstance(msg_json, dict):
                ...
                # 记录日志
                logger.bind(tag=TAG).info(f"收到{message_type}消息：{message}")
                ...
                if handler:
                    ...
                    logger.bind(tag=TAG).error(f"收到未知类型消息：{message}")
            # 处理纯数字消息
            elif isinstance(msg_json, int):
                logger.bind(tag=TAG).info(f"收到数字消息：{message}")
                await conn.websocket.send(message)

        except json.JSONDecodeError:
            # 非JSON消息直接转发
            logger.bind(tag=TAG).error(f"解析到错误的消息：{message}")
            await conn.websocket.send(message)
